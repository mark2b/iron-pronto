// DecodeIR.cpp : Defines the entry point for the DLL application.
//
...
#else
		/* Instead of reporting OBC in two separate values, just report it as 16-bits */
		*pOBC = (OBC1 << 8) | OBC2;
		pHex[0] = OBC1;
		pHex[1] = OBC2;
#endif
...
int get_hex_code2(char *pronto, int& frequency, int& intro_length, int& rep_length, int data[])
{
	int type; 
	const char * tok = " \t";
	char * s;
	...
	int fcode;
	s = strtok(NULL, tok);
	fcode = strtoul(s, NULL, 16);
	frequency = (int) (1000000.0/((double)fcode * 0.241246)); 
	s = strtok(NULL, tok);
	intro_length = strtoul(s, NULL, 16);
	s = strtok(NULL, tok);
	rep_length = strtoul(s, NULL, 16);
...
{
	get_hex_code2(pronto, *frequency, *intro_length, *rep_length, data);
}
